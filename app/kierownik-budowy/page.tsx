...
export default function KierownikBudowyPage() {
  const [companies, setCompanies] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // SZUKAMY BRANŻY: 'kierownik-budowy' (zgodnie z ID w formularzu dołączania)
    const q = query(
      collection(db, 'zgloszenia_firm'),
      where('industry', '==', 'kierownik-budowy'),
      ...
      const fetched = snap.docs.map(doc => {
        ...
        return {
          ...
          features: data.features || [
            'Nadzór i prowadzenie dziennika',
            'Odbiory techniczne budynków',
            'Doradztwo materiałowe i optymalizacja'
          ]
        };
      });
  ...
  return (
    <> 
      {/* 1. HERO SECTION - TECHNICZNY PRESTIŻ */}
      <section className="relative bg-[#0f172a] text-white pt-40 pb-40 px-6 overflow-hidden italic">
        
        {/* TŁO FOTO - Twoje nowe zdjęcie */}
        <div className="absolute inset-0 opacity-40 pointer-events-none">
          <img 
            src="/img/kierownik-budowy-nieporet-mistrzowie regionu.webp" 
            alt="Kierownik Budowy Nieporęt - Nadzór Inwestorski 2026"
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <span className="text-[#d4af37] font-black uppercase tracking-[0.6em] text-[10px] mb-6 block">Budownictwo • Bezpieczeństwo • Nieporęt 2026</span>
          <h1 className="text-5xl md:text-7xl font-black mb-8 tracking-tighter leading-[0.8] uppercase italic text-white">
            MISTRZOWIE <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] via-[#b8860b] to-[#d4af37]">NADZORU</span>
          </h1>
          <p className="text-slate-300 text-lg md:text-xl mb-12 max-w-3xl mx-auto leading-relaxed font-light">
            Niezależni <span className="text-white font-medium">Kierownicy Budowy</span> w Gminie Nieporęt. Eksperci, którzy dopilnują jakości Twojego domu od fundamentów aż po sam dach.
          </p>
        </div>
      </section>
      ...
      <div className="bg-[#f8fafc] text-[#1e293b] rounded-t-[4rem] -mt-20 relative z-20 shadow-[0_-20px_60px_rgba(0,0,0,0.4)] pb-32">
        ...
        <section className="px-6 pt-12"> 
          <div className="max-w-6xl mx-auto bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100">
            <h2 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] text-center mb-8 italic">Nadzór budowlany w miejscowościach:</h2>
            <div className="flex flex-wrap justify-center gap-3">
              {['Nieporęt', 'Białobrzegi', 'Beniaminów', 'Stanisławów', 'Józefów', 'Rynia', 'Wólka Radzymińska'].map((city) => (
        ...
        <section id="ranking" className="py-24 max-w-6xl mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-6xl font-black text-[#0f172a] tracking-tighter uppercase italic">Lista Ekspertów</h2>
            <div className="h-2 w-24 bg-[#d4af37] mx-auto rounded-full mt-4 shadow-[0_0_15px_rgba(212,175,55,0.5)]"></div>
          </div>
          
          <div className="space-y-12">
            {loading ? (
              <div className="text-center py-20 animate-pulse text-slate-300 font-black uppercase tracking-widest">Weryfikujemy uprawnienia kierowników...</div>
            ) : (
              <>
                ...
                {placeholders.map((spot) => (
                  <div key={`placeholder-${spot}`} className="bg-white/50 border-4 border-dashed border-slate-200 rounded-[3rem] p-16 flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-8 opacity-60">
                    <div>
                      <h3 className="text-2xl font-black text-[#0f172a] uppercase italic mb-2 tracking-tighter">Miejsce dla inżyniera</h3>
                      <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em]">Zajmij wolne miejsce w kategorii Kierownik Budowy</p>
                    </div>
        ...
        <section className="py-24 bg-white border-y border-slate-100">
          <div className="max-w-4xl mx-auto px-6 font-montserrat">
            <h2 className="text-3xl md:text-5xl font-black text-[#0f172a] mb-16 tracking-tighter uppercase italic text-center">
              Dlaczego kierownik budowy jest kluczowy?
            </h2>
            <div className="grid gap-10">
              <div className="bg-slate-50 p-10 rounded-[3rem] border-l-8 border-[#d4af37]">
                <h3 className="text-2xl font-black text-[#0f172a] mb-4 uppercase italic">1. Kontrola Wykonawców</h3>
                <p className="text-slate-500 text-sm leading-relaxed italic">
                  Kierownik budowy to Twoje oczy i uszy na placu boju. Pilnuje, aby ekipy budowlane nie oszczędzały na materiale i trzymały się projektu technicznego.
                </p>
              </div>
              <div className="bg-slate-50 p-10 rounded-[3rem] border-l-8 border-[#0f172a]">
                <h3 className="text-2xl font-black text-[#0f172a] mb-4 uppercase italic">2. Bezpieczeństwo i Prawo</h3>
                <p className="text-slate-500 text-sm leading-relaxed italic">
                  Prawidłowo prowadzony dziennik budowy oraz fachowe odbiory etapów to fundament spokojnego użytkowania domu przez lata i bezproblemowego zakończenia inwestycji.
                </p>
              </div>
            </div>
          </div>
