...
export default function KoszenieTrawyPage() {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const fetched = snap.docs.map(doc => {
        ...
        return {
          ...
          reviewsCount: data.reviewsCount || Math.floor(Math.random() * 12) + 4,
          features: data.features || [
            'Koszenie trawników i nieużytków',
            'Wertykulacja i nawożenie',
            'Utylizacja skoszonej trawy'
          ]
        };
      });
    ...
    <> 
      ...
      <section className="relative bg-[#0f172a] text-white pt-40 pb-40 px-6 overflow-hidden italic font-montserrat">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <span className="text-[#d4af37] font-black uppercase tracking-[0.6em] text-[10px] mb-6 block">Ogród i Zieleń • Nieporęt 2026</span>
          <h1 className="text-6xl md:text-9xl font-black mb-8 tracking-tighter leading-[0.8] uppercase italic">
            MISTRZOWIE <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] via-[#b8860b] to-[#d4af37]">OGRODÓW</span>
          </h1>
          <p className="text-slate-400 text-lg md:text-xl mb-12 max-w-3xl mx-auto leading-relaxed font-light">
            Perfekcyjny trawnik w <span className="text-white font-medium">Gminie Nieporęt</span>. Wybraliśmy fachowców, którzy zadbają o Twoją zieleń z chirurgiczną precyzją i pasją.
          </p>
        </div>
      </section>
      ...
      <div className="bg-[#f8fafc] text-[#1e293b] rounded-t-[4rem] -mt-20 relative z-20 shadow-[0_-20px_60px_rgba(0,0,0,0.4)] pb-32">
        ...
        <section className="px-6 pt-12"> 
          <div className="max-w-6xl mx-auto bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100">
            <h2 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] text-center mb-8 italic">Obsługujemy całą gminę:</h2>
            <div className="flex flex-wrap justify-center gap-3">
              {['Nieporęt', 'Białobrzegi', 'Zegrze', 'Rynia', 'Józefów', 'Wieliszew', 'Legionowo'].map((city) => (
        ...
        <section id="ranking" className="py-24 max-w-6xl mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-6xl font-black text-[#0f172a] tracking-tighter uppercase italic">Ranking Ekspertów Zieleni</h2>
            <div className="h-2 w-24 bg-[#d4af37] mx-auto rounded-full mt-4 shadow-[0_0_15px_rgba(212,175,55,0.5)]"></div>
          </div>
          
          <div className="space-y-12">
            {loading ? (
              <div className="text-center py-20 animate-pulse text-slate-300 font-black uppercase tracking-widest">Szukamy najlepszych ogrodników...</div>
            ) : (
              <>
                ...
                {placeholders.map((spot) => (
                  <div key={`placeholder-${spot}`} className="bg-white/50 border-4 border-dashed border-slate-200 rounded-[3rem] p-16 flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-8 opacity-60">
                    <div>
                      ...
                      <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em]">Zostań liderem pielęgnacji ogrodów w regionie</p>
                    </div>
        ...
        <section className="py-24 bg-white border-y border-slate-100">
          <div className="max-w-4xl mx-auto px-6 font-montserrat">
            <h2 className="text-3xl md:text-5xl font-black text-[#0f172a] mb-16 tracking-tighter uppercase italic text-center">
              Jak utrzymać trawnik w standardzie Premium?
            </h2>
            <div className="grid gap-10">
              <div className="bg-slate-50 p-10 rounded-[3rem] border-l-8 border-[#d4af37]">
                <h3 className="text-2xl font-black text-[#0f172a] mb-4 uppercase italic">1. Regularność i Wysokość Koszenia</h3>
                <p className="text-slate-500 text-sm leading-relaxed italic">
                  Kluczem do gęstego, zielonego dywanu jest systematyczność. Nasi Mistrzowie zalecają koszenie nie rzadziej niż raz w tygodniu, usuwając jedynie 1/3 wysokości źdźbła. Dzięki temu trawa nie jest stresowana, a Twój ogród zawsze wygląda jak pole golfowe.
                </p>
              </div>
              <div className="bg-slate-50 p-10 rounded-[3rem] border-l-8 border-[#0f172a]">
                <h3 className="text-2xl font-black text-[#0f172a] mb-4 uppercase italic">2. Profesjonalny Sprzęt i Noże</h3>
                <p className="text-slate-500 text-sm leading-relaxed italic">
                  Tępe noże zwykłych kosiarek szarpią trawę, co prowadzi do jej żółknięcia i chorób. Firmy z naszego rankingu korzystają z profesjonalnych maszyn z regularnie ostrzonymi nożami, co gwarantuje czyste cięcie i szybką regenerację trawnika.
                </p>
              </div>
            </div>
          </div>
