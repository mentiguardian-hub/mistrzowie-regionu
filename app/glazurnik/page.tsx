...
export default function GlazurnikPage() {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const fetched = snap.docs.map(doc => {
        ...
        return {
          ...
          reviewsCount: data.reviewsCount || Math.floor(Math.random() * 20) + 5,
          features: data.features || [
            'Układanie wielkoformatowe (spieki)',
            'Hydroizolacje łazienek i tarasów',
            'Precyzyjne szlifowanie narożników 45°'
          ]
        };
      });
  ...
  return (
    <> 
      {/* 1. HERO SECTION - ZMNIEJSZONY NAPIS */}
      <section className="relative bg-[#0f172a] text-white pt-40 pb-40 px-6 overflow-hidden italic font-montserrat">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <span className="text-[#d4af37] font-black uppercase tracking-[0.6em] text-[10px] mb-6 block">Architektura • Wnętrza 2026</span>
          {/* ZMNIEJSZONA CZCIONKA: md:text-7xl zamiast 9xl */}
          <h1 className="text-5xl md:text-7xl font-black mb-8 tracking-tighter leading-[0.85] uppercase italic pr-8">
            MISTRZOWIE <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] via-[#b8860b] to-[#d4af37]">GLAZURY</span>
          </h1>
          <p className="text-slate-400 text-lg md:text-xl mb-12 max-w-3xl mx-auto leading-relaxed font-light">
            Wybraliśmy najlepszych glazurników w <span className="text-white font-medium">Gminie Nieporęt</span>. Precyzja, która definiuje nowoczesne wnętrza.
          </p>
        </div>
      </section>
      ...
      <div className="bg-[#f8fafc] text-[#1e293b] rounded-t-[4rem] -mt-20 relative z-20 shadow-[0_-20px_60px_rgba(0,0,0,0.4)] pb-32">
          ...
          <div className="max-w-6xl mx-auto">
            <div className="bg-white rounded-[4rem] p-8 md:p-12 shadow-[0_20px_60px_rgba(0,0,0,0.3)] border-4 border-[#d4af37] flex flex-col md:flex-row items-center gap-12">
              ...
              <div className="relative w-full md:w-1/2 aspect-[4/3] rounded-[2rem] overflow-hidden border-8 border-white shadow-[0_10px_30px_rgba(212,175,55,0.4)] bg-slate-100">
                {/* UWAGA: Sprawdź czy plik jest w public/img/nazwa.webp */}
                <Image 
                  src="/img/glazurnik-nieporet-ukladanie-plytek-wielkoformatowych.webp" 
                  alt="Profesjonalny glazurnik Nieporęt" 
                  width={1200}
                  height={900}
                  priority
              ...
              <div className="w-full md:w-1/2 text-left space-y-6">
                <span className="text-[#d4af37] font-black uppercase tracking-[0.6em] text-[10px] block">Certyfikacja Jakości</span>
                <h3 className="text-3xl md:text-5xl font-black text-[#0f172a] tracking-tighter uppercase italic leading-[0.9]">
                  Sztuka <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] via-[#b8860b] to-[#d4af37]">PERFEKCJI</span>
                </h3>
                <p className="text-slate-500 text-sm leading-relaxed italic font-light">
                  Najwyższy standard układania glazury. Nasi partnerzy Platinum gwarantują rzemiosło zgodne z najnowszymi trendami i normami budowlanymi.
                </p>
              </div>
            </div>
          </div>
        ...
        <section id="ranking" className="py-24 max-w-6xl mx-auto px-6">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-6xl font-black text-[#0f172a] tracking-tighter uppercase italic">Ranking Ekspertów Glazury</h2>
            <div className="h-2 w-24 bg-[#d4af37] mx-auto rounded-full mt-4 shadow-[0_0_15px_rgba(212,175,55,0.5)]"></div>
          </div>
          
          <div className="space-y-12">
            {loading ? (
              <div className="text-center py-20 animate-pulse text-slate-300 font-black uppercase tracking-widest">Weryfikujemy portfolio...</div>
            ) : (
              <>
                ...
                {placeholders.map((spot) => (
                  <div key={`placeholder-${spot}`} className="bg-white/50 border-4 border-dashed border-slate-200 rounded-[3rem] p-16 flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-8 opacity-60">
                    <div>
                      ...
                      <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em]">Zajmij wolne miejsce w rankingu</p>
                    </div>
